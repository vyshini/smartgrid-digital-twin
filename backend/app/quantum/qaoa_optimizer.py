...
VERSION NOTE: uses Qiskit's own StatevectorSampler (qiskit.primitives), not
Aer's SamplerV2. An earlier version of this file used Aer's SamplerV2 with
a transpiler/pass-manager (required for any V2 sampler other than
StatevectorSampler) — but this failed in practice with
"QAOA.__init__() got an unexpected keyword argument 'transpiler'", because
that parameter was only added in a newer qiskit_algorithms release than
what's actually installed. Rather than chase an exact version pin,
StatevectorSampler sidesteps the whole issue: it needs no transpiler at
all, and is Qiskit's own documented canonical way to run QAOA. For a small,
noiseless circuit at this qubit count, it performs the same exact
statevector computation Aer's simulator would — this is a real, deliberate
tradeoff (not literally "Aer" per the original spec) made for robustness
against version fragility, not a silent shortcut.

Required packages: qiskit, qiskit-algorithms (qiskit-aer no longer required
for this file specifically, though still listed in requirements.txt for
potential future use / consistency with the original tech stack).
...
from app.quantum.hamiltonian_builder import DispatchProblem, build_qubo, decode_solution, qubo_objective, qubo_to_ising
...
if __name__ == "__main__":
    ...
    result = run_qaoa(problem, reps=1, shots=512, maxiter=50)
